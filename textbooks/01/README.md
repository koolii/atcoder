...
可能かどうかを判定する問題では、まず **不可能な場合を列挙していくことが有効** でその時は **極端な場合を考える** と具体例を作るのにも有効

### 毎回原点スタートとしてよい

点(Xi, Yi)から点(Xi+1, Yi+1)への移動は点(0, 0)から点(Xi+1 - Xi, Yi+1 - Yi)への移動と同一視できる

このように、スタート地点が(0, 0)となるように調整することで考えやすくなる、様々な問題で活用できるテクニック

※ X = Xi+i - Xi, Y = Yi+1 - Yi, T = Ti+1 - Ti と置くと、
**点(0, 0)を出発して、ちょうど T 秒後に、点(X, Y)にいることは可能かどうかを判定せよ** 等を解決できる

### 必要条件と十分条件を意識する

「〜が可能かどうかを判定せよ」関連の問題が多く、有力なアプローとの一つとして

- 条件 A を満たさなければ不可能
- 条件 B を満たさなければ不可能
- ...

というように不可能な場合を列挙していくことです。
条件 A,B…は可能であるための必要条件であり、逆に「その条件を満たせば可能である」時の条件は十分条件であるという

## 6-2 再帰

### 再帰関数を用いた全探索

全探索を自在に書けることは、今後あらゆるアルゴリズムを学ぶ時に重要な基礎となります。
再帰関数を用いることで明快な記述のできる全探索アルゴリズムは様々ななものが考えられます。
ここでは、「n 重 for 文を用いた全探索」を再帰関数で記述してみる。
...
7 重の for 文は 2^7 通りの場合を全て走査するものとなっています。
つまり、0 と 1 のみからなる長さ 7 の数列（例えば [0, 1, 1, 0, 1, 0, 1] etc...) を全列挙するものです。
しかし上記は見た目が大変で、バグを生みやすい。そして for 文を何重にすればよいのか動的に決めたい場合もある。
...
Ai=0 を「i 番目の品物を選ばない」、Ai=1 を「i 番目の品物を選ぶこと」に対応させる
A=[1,1,0,1,0]は 5 個の品物の内、0,1,3 を選ぶことに対応する
よって、「0 と 1 のみからなる長さ N の数列」(2^N 通り)をすべて列挙できれば、ナップサック問題が解けることになります。

**「0 と 1 のみからなる長さ N の数列」を全探索するために、for 文を重ねたいが、N が動的な為に for 文を何個書けばよいのか分かりません。そこで再帰関数の出番になり、下記に全探索(全パターンの列挙を記述する)**

_./6.2.9.png_ も役立つ為確認すること
...
def rec(A):
  ...
  for v in range(2):
    A.append(v) # Aに要素を加える
    rec(A) # 再帰的に呼び出す
    # サイズNまでの夫々の階層で 0と1のケースと2^Nのパターン総当りさせる必要がある
    # 0が終わったら、次に1のケースで総当りをさせる必要がある
    # 悩んで一度 pop()させる理由は理解できた
    # が、また分からなくなったらサイズN=3の最後の階層から抜け出した時のサイズN=2の時のループ処理の事を考える
    A.pop()
```

Python3 では、0 と 1 のみからなる長さ N の数列を列挙する方法として、他に **itertools.product** を用いるものがあります。いろいろな関数があり便利
...
全探索が完了したため、実際にナップサック問題を解く、計算量は O(2^N)

_./6_2_ex1_knapsack.py_ を参照
